  ...
  try {
    api.onmessage = function(msg, data)  {
      if ("event" in data) {
        ...
        if ("event" in data && data.event == "UpdateDroneInfo") {
          // console.log(data);
          // console.log(data.details);
          updateDroneInfoUI(data.details);
        }
...
// ////////////////////////////
function updateDroneInfoUI(droneDetails) {
  let charge = Number(droneDetails.droneBatteryLevel).toFixed(2);
  let availability = droneDetails.available ? 'Available' : 'Not Available';

  document.getElementById('drone-name').innerText = 'Name: ' + droneDetails.droneName;
  document.getElementById('drone-battery').innerText = 'Battery Level: ' + charge + '%';
  document.getElementById('drone-mode').innerText = 'Mode: ' + droneDetails.mode;
  document.getElementById('drone-availability').innerText = 'Availability: ' + availability;
}
// //////////////////////////////////////


// function createEntityInfoUI(entityDetails) {
  
//   var infoDiv = document.createElement('div');  // Create the container div
//   infoDiv.id = entityDetails.id + '-info';      // unique id
//   infoDiv.classList.add('entity-info');

//   // Make and append the name p element
//   var nameP = document.createElement('p');
//   nameP.innerText = 'Name: ' + entityDetails.name;
//   infoDiv.appendChild(nameP);

//   // Append the container to the notification bar
//   document.getElementById('notification-bar').appendChild(infoDiv);
// }
